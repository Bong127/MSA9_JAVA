...
public class ByteTextInput {
	...
	public static void main(String[] args) {
		...
		try {
			...
			int byteTxt = fis.read(data);		// 바이트 배열에 데이터 입력
			System.out.println(byteTxt);
			// 바이트 배열 data를 인덱스 0부터 마지막 바이트(byteTxt)까지 UTF-8 인코딩 타입의 문자열 반환
			String text = new String(data, 0, byteTxt, StandardCharsets.UTF_8);
